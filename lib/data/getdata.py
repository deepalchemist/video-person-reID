...
class Mars(object):
    ...
    """
    root = '/mnt/data2/caffe/person_reid/mars'
...
class DukeMTMCVidReID(object):
    ...
    """
    root = '/mnt/data2/caffe/person_reid/'
    ...
    def __init__(self, min_seq_len=0, **kwargs):
        ...
        train, num_train_tracklets, num_train_pids, num_train_imgs = \
            self.process_dir(self.train_dir, self.split_train_json_path, relabel=True)
        query, num_query_tracklets, num_query_pids, num_query_imgs = \
            self.process_dir(self.query_dir, self.split_query_json_path, relabel=False)
        gallery, num_gallery_tracklets, num_gallery_pids, num_gallery_imgs = \
            self.process_dir(self.gallery_dir, self.split_gallery_json_path, relabel=False)
    ...
    def process_dir(self, dir_path, json_path, relabel):
        if osp.exists(json_path):
            split = util.read_json(json_path)
        ...
        util.write_json(split_dict, json_path)
...
class iLIDSVID(object):
    ...
    def __init__(self, split_id=0):
        ...
        self._prepare_split()
        splits = util.read_json(self.split_path)
    ...
    def _download_data(self):
        ...
        util.mkdir_if_missing(self.root)
    ...
    def _prepare_split(self):
        if not osp.exists(self.split_path):
            ...
            util.write_json(splits, self.split_path)
