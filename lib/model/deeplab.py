...
if __name__ == "__main__":
    import time
    model = DeepLabV2(
        n_classes=21, n_blocks=[3, 4, 6, 3], atrous_rates=[6, 12, 18, 24]
    )
    # model.freeze_bn()
    # print(model)
    num_params = sum([m.numel() for m in model.parameters() if m.requires_grad]) / 1e6
    print("number parameters: {:.3f}M".format(num_params))

    image = torch.ones(1, 3, 513, 513)
    print("input:", image.shape)
    t=time.time()
    output = model(image)
    end_t = time.time()-t
    print("output: {}, and time taken {}".format(output.shape, end_t))
