...
class TSN(nn.Module):
    ...
    def _prepare_base_model(self, base_model, num_class, conv5_stride=1, bn=False, pool_type='avg'):
        ...
        if 'resnet' in base_model:
            ...
            self.base_model = getattr(lib_resnet, base_model)(
                pretrained=True,
            ...
            self.fc = nn.Linear(self.num_features, num_class, bias=True)
    ...
    def forward(self, input, no_reshape=False):
        ...
        if self.is_shift and self.temporal_pool:
            ...
            predict = predict.view((-1, self.num_segments) + predict.size()[1:])
            feature = base_out.view((-1, self.num_segments) + base_out.size()[1:])
